...
function App() {
  ...
  const [singleRangeMinMax, setSingleRangeMinMax] = useState({ min: 0, max: 0 });
  ...
  const filterRangePrice = value => {
    ...
    const filtered = books.masterBooks.filter(b => b.price <= Math.ceil(max / 100) && b.price >= Math.floor(min / 100));
    setBooks({
      showBooks: filtered,
    ...
    <Books.Provider value={books}>
      <div className="App col top">
        <div className="books">
          ...
          {/* <div className="books_price-slider">
            <div className="books_price-slider_value">
              Max kaina: {(singleSlider / 100).toFixed(2)} €
              </div>
            <Slider min={singleSliderMinMax.min} max={singleSliderMinMax.max} value={singleSlider} 
            onChange={onSingleSliderChange} />
          </div> */}

          <div className="books_price-slider">
            <div className="books_price-slider__value">
              <span>Min kaina: {(Math.min(singleRange[0], singleRange[1]) / 100).toFixed(2)} €</span>
              <span>Max kaina: {(Math.max(singleRange[0], singleRange[1]) / 100).toFixed(2)} €</span>
            </div>
            <Range min={singleRangeMinMax.min} max={singleRangeMinMax.max} value={singleRange} onChange={onSingleRangeChange} />
          </div>
